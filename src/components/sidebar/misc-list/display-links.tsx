...
import DeletedForms from "@/components/deleted-forms";
...
const DisplayLinks = ({forms} : {forms : { id: string, title: string }[]}) => {

    const[isOpen, setIsOpen] = useState(false)

    return <div className={"mt-2"}>
        {
            isOpen && <DeletedForms onClose={() => {
                setIsOpen(false);
            }} forms={forms}/>
        }
        <div className={""}>
            ...
            <div className={"flex flex-col mt-2"}>
                {
                    links[0].sublinks.map((link,i) => (
                        <Link key={i} href={"/dashboard"}
                              className={"flex items-center gap-x-2 text-gray-7 text-[11px] hover:bg-gray-100 rounded-md transition duration-200 pl-1 pr-1 hover:text-black"} onClick={() => {
                            if (link.name === "Trash") setIsOpen(prevState => !prevState)
                        }}>
